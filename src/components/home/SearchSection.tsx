...
const SearchSection: React.FC = () => {
  ...
  const [programsList, setProgramsList] = useState<{ name: string; path: string }[]>([]);

  const handleLevelSelect = (level: string) => {
    setSelectedLevel(level);
    setProgramsList(programs[level]);
  ...
  const handleSearch = () => {
    if (selectedProgram && selectedLevel) {
      const program = programs[selectedLevel].find(
        (prog) => prog.name === selectedProgram
      );
      if (program) {
        window.location.href = program.path;
      }
      ...
      <div className="row align-items-center">
        ...
        <div className="col-md-6 col-sm-12 mb-2">
          <select
            ...
            disabled={!selectedLevel} // Disabled until a school is selected
          >
            <option value="" disabled>
              {selectedLevel ? "Select a program" : "Select a school first"}
            </option>
        ...
        <div className="col-md-2 col-sm-12 mb-2">
          <button
            ...
            className="cs_btn cs_style_1 cs_type_btn"
            onClick={handleSearch}
            disabled={!selectedProgram} // Disabled until a program is selected
          >
            <span>Search</span>
          </button>
        </div>
